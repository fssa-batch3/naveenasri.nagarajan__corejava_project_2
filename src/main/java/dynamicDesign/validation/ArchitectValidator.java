...
public class ArchitectValidator {
	...
	public static boolean validateArchitectID(int architectID) {
		if (architectID < 0) {
			System.out.println("Architect ID is invalid: Negative value.");
			return false;
		}

		String regexArchitectID = "^[0-9]+$";

		// Check if the architect ID matches the regex pattern
		...
		if (match) {
			...
			System.out.println("Architect ID is invalid: Only non-negative numbers are allowed.");
		}
	...
	public static boolean validateName(String name) {
		...
		// The name should start with a letter and can contain letters, digits, or
		// underscores.
	...
	public static boolean validatePhoneNumber(String phoneNumber) {
		...
		// Create a Matcher object to match the provided phone number against the
		// pattern
		...
		// Return the match result, which is true if the phone number is valid, false
		// otherwise
		return isMatch;
	}
	...
	public static boolean validateAddress(String address) {
		...
		// For example, the following regex allows any non-empty string as a valid
		// address.
		String regex = ".+";
